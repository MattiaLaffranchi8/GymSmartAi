...
window.manualLogin = async function () {
    ...
    if (!id) { input.classList.add('input-error'); return; }
    
    btn.classList.add('loading');
    processLogin(id).finally(() => btn.classList.remove('loading'));
};
...
window.generatePDFReport = async function() {
    ...
    if (canvas) doc.addImage(canvas.toDataURL(), 'PNG', 20, 70, 170, 80);
...
function syncProfile() {
    db.ref('atleti/' + currentUserId).on('value', snap => {
        ...
        document.getElementById('health-status').innerText = `Status: ${d.salute || 'OK'}`;
    });
}
